...
: ?@ dup if @ then ; 		\ check "NULLS (end of questions or rules) before fetch
: .sym dup 0<> if 2 cells - >name type else drop ." Unknown" then ;
...
: ask .sym ."  (type y/n/why?)" ;
: result ." Answer: " rule @ .sym ;
: respond nextq ?@ dup if ask else drop result then ;
\ User words
: y -1 q @ ! respond ;
: n 1 q @ ! respond ;
: why? rule @ dup .sym begin dup q cell+ <> while cr 9 emit dup @ .sym cell+ repeat drop ;
